...
class SwissArmyLayer(nn.Module):
    ...
    def __init__(self, t_seq_bits
        ...
        if num_layers <=1:
            #only do one layer.  the outp
            self.layers = nn.ModuleList(
            [nn.Linear(hidden_dim, output_dim)])
        self.layers = nn.ModuleList(
            [nn.Linear(hidden_dim, hidden_dim) for i in range(num_layers-1)])  # should work with zero too.
...
class SeqEncoder(nn.Module):
    def __init__(self, t_seq_bits, t_seq_len, t_layer_dim, t_num_layers, fc_layers, encoder_layers, one_hot_vocab_len,
                 one_hot_embedding_dim, output_dim):
        ...
        input_embedding_dim = output_dim
        ##These lines removed, i don't wan't the growth since i want to do res stuff
        #if t_num_layers == 0:
    ...
    def forward(self, t_seq, one_hot_idx):
        ...
        full_embedding = self.initial_layer(t_seq, one_hot_idx=one_hot_idx)
        
        
        
        # Pass through each encoder layer
        for layer in self.encoder_layers:
            x = full_embedding.sum(dim=1, keepdim=True)
            x = x.expand(-1, self.seq_len, -1)
            x = layer(t_seq, one_hot_idx=one_hot_idx, input_embedding=x)
            full_embedding = full_embedding + x

        full_embedding = full_embedding.sum(dim=1, keepdim=True)
        full_embedding = full_embedding.squeeze() #final layer squozen for decoder input
